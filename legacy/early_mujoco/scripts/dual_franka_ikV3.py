...
if __name__ == "__main__":
    ...
    collision_pairs = [
        (
            mink.get_subtree_geom_ids(model, model.body("hand_l").id),
            mink.get_subtree_geom_ids(model, model.body("hand_r").id),
        ),
    ]
    ...
    target_position_right = np.array([0.3, -0.4, 0.5])  
    target_quaternion_right = np.array([0, -1, 0, 1])  
