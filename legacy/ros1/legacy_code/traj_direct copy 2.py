...
def call_compute_ik(group_name, current_position, target_z, current_orientation):
    ...
    keeping the orientation unchanged by applying orientation constraints.
    """
    ...
    # Set the orientation (keep it the same as passed in current_orientation)
    ...
    # Add orientation constraints to prevent orientation changes
    orientation_constraint = OrientationConstraint()
    orientation_constraint.header.frame_id = "world"
    orientation_constraint.link_name = group_name + "_link7"  # Assuming the end effector is link7
    orientation_constraint.orientation.x = current_orientation[0]
    orientation_constraint.orientation.y = current_orientation[1]
    orientation_constraint.orientation.z = current_orientation[2]
    orientation_constraint.orientation.w = current_orientation[3]
    orientation_constraint.absolute_x_axis_tolerance = 0.01
    orientation_constraint.absolute_y_axis_tolerance = 0.01
    orientation_constraint.absolute_z_axis_tolerance = 0.01
    orientation_constraint.weight = 1.0  # Strong constraint to keep orientation fixed
    
    # Add the constraint to the IK request
    ik_request.ik_request.constraints.orientation_constraints.append(orientation_constraint)
...
def send_trajectory():
    ...
    # Current positions and orientations (set individually for both arms)
    ...
    # Ensure independent orientations for both arms
    ...
    while current_left_z < final_target_z and current_right_z < final_target_z:
        ...
        # Call compute_ik for both arms, keeping their independent orientations
