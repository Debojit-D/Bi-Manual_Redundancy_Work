...
# Run simulation for stabilization before retrieving site values
for _ in range(100):
...
# Ensure reference sites exist before use
site_names = [model.site(i).name for i in range(model.nsite)]
assert "attachment_site_left" in site_names, "ERROR: 'attachment_site_left' not found!"
assert "attachment_site_right" in site_names, "ERROR: 'attachment_site_right' not found!"
assert "site_left" in site_names, "ERROR: 'site_left' not found!"
assert "site_right" in site_names, "ERROR: 'site_right' not found!"

# Get site IDs (for measured values)
    ...
    with mujoco.viewer.launch_passive(model=model, data=data, show_left_ui=False, show_right_ui=False) as viewer:
        ...
        for wp_index in range(len(left_waypoints)):
            ...
            if wp_index == 1:
                configuration.update(data.qpos)
                posture_task.set_target_from_configuration(configuration)
                pos_threshold, ori_threshold, max_iters = 0.008, 0.008, 50
            ...
            while not reached and viewer.is_running():
                ...
                for i in range(max_iters):
                    vel = mink.solve_ik(configuration, tasks, rate.dt, solver, 5e-3)  # Increased dt
                    ...
                    err_pos_left, err_pos_right = np.linalg.norm(measured_left_pos - current_left_pos), np.linalg.norm(measured_right_pos - current_right_pos)
                    err_ori_left, err_ori_right = quaternion_error(measured_left_quat, current_left_quat), quaternion_error(measured_right_quat, current_right_quat)

                    if err_pos_left <= pos_threshold and err_ori_left <= ori_threshold and err_pos_right <= pos_threshold and err_ori_right <= ori_threshold:
                        reached = True
                        break
        ...
        while viewer.is_running():
            mujoco.mj_step(model, data)
            viewer.sync()
            rate.sleep()  # Keep updating to prevent freezing
