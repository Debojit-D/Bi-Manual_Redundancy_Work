...
mocap_body_id = model.body(mocap_body_name).mocapid  # Get mocap body ID properly
...
relative_rotation = Rotation.from_euler('z', 30, degrees=True).as_quat()
...
def update_mocap_target(data, target_pos, target_quat):
    """Moves the mocap body to the target pose for IK"""
    data.mocap_pos[mocap_body_id] = target_pos
    data.mocap_quat[mocap_body_id] = target_quat
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    while current_time < total_time:
        ...
        update_mocap_target(data, x_R, q_R)
        ...
        try:
            ...
            # Ensure the configuration updates properly
            configuration.update(joint_positions)
