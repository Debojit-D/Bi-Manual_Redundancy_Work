...
def compute_next_phi(A, J_h, current_index, delta_t=0.1, K_p=10):
    global current_joint_states
...
def command_joint_states(joint_states):
    ...
    # Both arms joint states (assume 14 joints: 7 for left, 7 for right)
    both_arms_joint_states = joint_states[:14]

    # Set the joint positions for both arms
    both_arms_group.set_joint_value_target(both_arms_joint_states)

    # Plan and execute the movement
    plan = both_arms_group.plan()
    
    # If a valid plan exists, execute it
    if plan:
        both_arms_group.execute(plan, wait=True)
    else:
        rospy.logwarn("Planning failed. No valid plan found for the given joint states.")
