...
This module is NOT one of the manuscript's numbered equations. It lives
beside, rather than inside, the paper's ``ManipulabilityOptimizer`` and
generalizes Equations (16)/(17) into four exploratory combinations. It tests
two independent choices:

1. capability matrix: ``A A.T`` (as in Eq. 17) or its pseudoinverse
   (as in Eq. 16);
2. distance direction: minimize or maximize the normalized Frobenius distance.

Only ``FORCE_MINIMIZE`` reproduces Equation (16)'s direct force-space
objective; only ``VELOCITY_MAXIMIZE`` reproduces Equation (17)'s indirect
velocity-dual objective. ``FORCE_MAXIMIZE`` and ``VELOCITY_MINIMIZE`` are
experimental controls with no corresponding paper equation.
