...
class CooperativeManipulationKinematics:
    ...
    def grasp_matrix(self, data):
        """Return the 6x12 spatial full-contact grasp matrix ``G`` (Eq. 1, 9).

        Two contact points (``k=2``), full contact model (``p=6``), task
        space ``m=6``, so ``G in R^(m x kp) = R^(6x12)``. Appears as ``G^T``
        in the contact-compatibility constraint ``J_H phi_dot = G^T q_dot``
    ...
    def paper_object_velocity_map(self, data):
        ...
        necessarily square. This is the raw map used to build the
        manipulability objectives (Eq. 12-17) -- see
        :meth:`tracking_object_velocity_map` for the separate map used by
        the controller's primary tracking term.
        """
    ...
    def null_space_projector(self, data):
        ...
        motion unaffected: ``J_H @ [(I - J_H^dagger J_H) phi_dot_opt] = 0``.
        """
        hand_jacobian = self.hand_jacobian(data)
