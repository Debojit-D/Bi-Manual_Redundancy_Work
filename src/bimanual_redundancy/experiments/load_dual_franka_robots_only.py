...
Normal launch (generate once, then reuse the cached sphere XML)::
    ...
    python -m bimanual_redundancy.experiments.load_dual_franka_robots_only

Select a straight-on front or overhead camera::

    python -m bimanual_redundancy.experiments.load_dual_franka_robots_only --front-view
    python -m bimanual_redundancy.experiments.load_dual_franka_robots_only --top-view
...
SOURCE_MODEL_PATH = (
    paths.ROBOT_MODELS_DIR / "franka_emika_panda" / "dual_panda_robots_only_scene.xml"
)
SPHERE_MODEL_PATH = SOURCE_MODEL_PATH.with_name(
    "dual_panda_robots_only_spherefit.xml"
)
SPHERE_GENERATOR_PATH = (
    paths.MUJOCO_CUROBO_BRIDGE_DIR / "demos" / "02_fit_mujoco_spheres.py"
)
