...
def compute_next_phi(A, J_h, current_index, positions, orientations, q_dot_d, delta_t=0.01, Kp=1.2):
    ...
    global current_joint_states, previous_phi, previous_previous_phi
    ...
    if previous_phi is not None and previous_previous_phi is not None:
        delta_previous = previous_phi - previous_previous_phi
        for i in range(len(phi_next)):
            if abs(phi_next[i] - previous_phi[i]) > 2 * abs(delta_previous[i]):
                phi_next[i] = previous_phi[i] + delta_previous[i]
    
    # Update previous phi states
    previous_previous_phi = previous_phi
    previous_phi = phi_t
