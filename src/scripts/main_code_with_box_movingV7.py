#### This is for directional force manipulability (2nd Formulation) optimization
...
def compute_A_directional_modification(G_matrix, Hand_Jacobian):
    """Compute matrix A using the first 3 rows of G_matrix."""
    if G_matrix is None or Hand_Jacobian is None:
        rospy.logwarn("G_matrix or Hand_Jacobian not received yet!")
        return None
    
    # Select the first 3 rows of G_matrix
    G_matrix_reduced = G_matrix[:3, :]
    
    G_inv_transpose = np.linalg.pinv(G_matrix_reduced).T
    A = np.dot(G_inv_transpose, Hand_Jacobian)
    
    return A


def compute_phi_dot_opt(A, optimization_type, gain=0.00000003):
    ...
    if optimization_type == "velocity_manipulability":
        ...
        F = np.array([-9.81*3, 0, 0])  # Assuming 6D force vector
...
def compute_next_phi(A, J_h, current_index, positions, orientations, q_dot_d, delta_t=0.01, K_p=1.0):
    ...
    phi_dot_opt = compute_phi_dot_opt(compute_A_directional_modification(G_matrix, Hand_Jacobian), "directional_force_manipulability")
