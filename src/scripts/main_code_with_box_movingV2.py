...
def quaternion_to_angular_velocity(q1, q2, dt):
    """Compute angular velocity from the change in two quaternions."""
...
def read_trajectory_and_compute_q_dot_d(csv_file, dt):
    """Read trajectory from CSV file and compute velocity."""
    trajectory_data = []
...
def grad_manipulability(A, W, delta=1e-6):
    """Compute the gradient of the manipulability measure W with respect to joint angles."""
...
def move_robot_and_save_joints(csv_file_name="joint_angles.csv"):
    """Move the robot along the trajectory and save the joint angles."""
    global left_arm_pub, right_arm_pub
    current_index = 0
